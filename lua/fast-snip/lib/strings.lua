...
String.format_with_delimiters = function(input, ranges, delimiters)
	local output = ""
	local start_index = 1

	for _, range in ipairs(ranges) do
		if #range == 4 then
			range = String.convert_4d_range_to_2d_range(input, range)
		end
...
function String.if_string_empty(str)
...
String.get_smallest_indent = function(lines, ignore_line)
	...
	for _, line in ipairs(lines) do
		...
		if not String.if_string_empty(line) and (spaces_len < smallest_indent) and (line ~= ignore_line) then
...
String.dedent_by = function(input, dedent_value)
...
String.dedent = function(input)
	...
	local smallest_indent = String.get_smallest_indent(lines)
...
function String.replace_range(input, replacement, range)
	local start_row, start_col, end_row, end_col = unpack(range)
	...
	for i, line in ipairs(lines) do
		if i > start_row and i < end_row then
			...
			if i == start_row and i == end_row then
				...
				if i == end_row then
					lines[i] = _end
					if String.if_string_empty(lines[i]) then
						table.insert(lines_to_delete, i)
					end
				end
			end
...
String.find_first_match_regex = function(strings, regex_pattern)
	strings = Table.to_table(strings)
	for index, str in ipairs(strings) do
		if vim.regex(regex_pattern):match_str(str) then
			return index
		end
	end
	return nil
...
String.find_first_match_lua = function(strings, lua_pattern)
	strings = Table.to_table(strings)
	for index, str in ipairs(strings) do
		local from, _ = str:find(lua_pattern)
		if from then
			return index
		end
	end
